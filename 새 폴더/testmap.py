...
class party:
    def __init__(self,potion,elix,stat1,stat2,stat3,stat4):
        self.potion = potion
        self.elix = elix
        ...
        self.mp4 = stat4[3]
    def party1(self):
        print(self.attack1,self.attack11,self.hp1,self.mp1)
        attack1 = 100+self.attack1
        attack11 = 150 + self.attack1
        hp1 = 500+ self.hp1
        mp1 = 300+ self.mp1
        return ["초코의용군",attack1, attack11, hp1, mp1]
    # def party2(self):
...
def battle(potion,elix,stat1):
    ch_monster=mob()
    partys=party(potion,elix,stat1)
    uy=partys.party1()
    hpmax=uy[2]
    ...
    while True:  # 싸움 반복문 열기
        ...
        if select == 1:  # 공격을 선택했을때
            ...
            if escape_rate <= 100:  # 도망 성공
                print("도망가는데 성공했습니다.")
                return 0, potion, elix
            else:  # 도망 실패
                print("도망가는데 실패했습니다.")
                print(f"{ch_monster[0]}이 {uy[0]}을 공격했습니다.")  # 몬스터가 의용이를 공격
                if elixir_status == 0:
...
def map():                                                              #맵총괄함수
    ...
    while (1):
        ...
        if map[col][row] == 3:                                             #몬스터 만나면 전투
            battlecnt+=1
            sign,potion,elix=battle(potion,elix,stat1)
            if sign==1:
                if random.randrange(0,2)==0:
                    for i in range (0,4):
                        stat1[i] += round(b[i+1]*(random.randrange(2,6)/100))
                    party(potion, elix, stat1)
